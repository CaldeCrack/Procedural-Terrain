...
var upper_bound := size / 2
var lower_bound := - upper_bound
var interval := range(lower_bound, upper_bound + 1)
var unload_dist := size * spacing
...
func _process(_delta: float) -> void:
	...
	if spawn_queue.is_empty():
		var player_grid := Vector3i(player.global_position / spacing)
		
		# Iterate grid around the player
		for x in interval:
			for y in interval:
				for z in interval:
					var grid_pos := player_grid + Vector3i(x, y, z)
					grid_pos *= spacing
					if not Global.spawned_positions.get(grid_pos, false):
